...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        box.innerHTML = (isMobileLayout() ? teamMobileOrder(employees) : employees).map((e, i) => teamCardHTML(e, i % 3)).join('');
        revealNew(box);
    } catch (err) {
        // Без сървър → резервен екип.
        const fb = (window.BH_FALLBACK && BH_FALLBACK.employees) || [];
        if (fb.length) {
            box.innerHTML = (isMobileLayout() ? teamMobileOrder(fb) : fb).map((e, i) => teamCardHTML(e, i % 3)).join('');
            revealNew(box);
        } else {
